...
| # | Mechanism | Evidence |
|---|---|---|
| 1 | Character snapshots already carry recursive contents | `CharacterItemMsg.Contents` (`CharacterItemMsg.cs:38`, recursive `[ProtoMember(7)]`); `RemoteInventoryServiceTests` and `NetPacketTests.CharacterData_EveryFieldFamily_RoundTrips` (the wire codec) already round-trip nested items |
