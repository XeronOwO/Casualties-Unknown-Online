...
- **Cross-player item use (give/feed/drink/wear/use an item on another player)** — **CLOSED (first slice + medicine + topical + opiates + limb-tool + component-bearing tools + wear, 2026-08-25)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) now covers drink/food, curated injectables, topical `ApplyToLimb`, opiates/antagonists, non-liquid limb tools, component-bearing limb tools, and native wearable placement. The additive `WornItem` result field carries the worn item to the target; no protocol bump. Remaining future slices: timed/component medicine, minigame-random tools, timed tools. See `docs/tech-decisions.md` #96/#98/#100/#102/#103/#105/#106.
