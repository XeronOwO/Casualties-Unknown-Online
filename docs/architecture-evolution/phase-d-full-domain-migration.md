...
| 2026-08-29 | Enemy target resolver extraction | `feac618` | 1769 tests green; build/format/architecture/event/entity/isolation/delivery gates pass | `EnemyTargetResolver` plus top-level `EnemyTarget` now own candidate-set building, find-back, and limb-index selection; `EnemyCombatDirector` keeps only the ordering/reporting responsibilities. Behavior-preserving. See `docs/selfchecks/phase-d-enemies-shadow-selfcheck.md`. |
