...
- **PVP** — **LOW (reprioritized 2026-08-25)**. No player-to-player damage domain today; defer until PvE, rules, and accept-first arbitration are stable. See §2.6.
...
- **In-world right-click player interaction menu** — **CLOSED (2026-08-23)**. Right-clicking near a remote player now opens a context menu reusing the Players-page action eligibility (Carry/Drop/Heal/Recruit/Take) plus an always-available "View items" fallback that opens the standalone player-interaction quick panel pinned to that member and expands its inventory (never the full Online window). It uses authoritative entity positions instead of the remote clones' disabled colliders. No protocol change. See `docs/selfchecks/character-attack-anim-and-player-context-menu-selfcheck.md` and `docs/tech-decisions.md` #84.
- **Cross-player item use (give/feed/drink/wear/use an item on another player)** — **CLOSED (through drinkable medicine, 2026-08-25)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) covers drink/food, curated injectables, topical `ApplyToLimb`, opiates/antagonists, component medicine, limb tools, component-bearing tools, native wearable placement, shrapnel/timed tools, timed/random injectable liquid medicine (`TimedBodyEffects`, additive), and drinkable timed/random/component medicine (`RemoteDrinkMedicineCatalog`: antirad, sleepingpills, painkillers, antibiotics, antidepressants, braingrow, mindwipe, keratinbooster, naltrexone, amiodarone, vasopressin, sodiumnitroprusside, mixed morphine). No remaining open rows in this family; the UI entry is drag/overlap release on the remote body (static Use buttons removed). See `docs/tech-decisions.md` #96-#110.
...
### Player interaction / piggyback follow-ups — 4 CLOSED (2026-08-26)
...
- **Drop/release floating body — CLOSED (2026-08-26, root-caused)**. The
  release path now empties the carried-driver's carrier id before Unity's
  deferred `Object.Destroy`, and every render-proxy/freeze guard treats a
  zero-carrier driver as already released. The previous restore re-enabled the
  frozen rigidbodies, but the deferred destroy left a one-frame window where
  the proxy path could run again and re-freeze the body — the "after Drop the
  character cannot move" symptom. New driver-state regression tests cover the
  release edge. See `docs/selfchecks/piggyback-drop-cleanup-selfcheck.md` and
  `docs/tech-decisions.md` #113.
...
- **Dedicated standalone player-interaction UI** — **CLOSED (2026-08-25)**. A hotkey-docked `OnlineUiQuickPanel` (default `[Session] InteractionPanelKey=F6`) shows the selected in-world remote player's status/inventory and all eligible co-op actions outside the full Online window; the target auto-follows the current/nearest remote and a compact selector handles multiple remotes. The in-world right-click context menu remains the transient cursor-based entry; its "View items" fallback now opens the quick panel pinned to the clicked remote and expands its inventory. No wire/protocol change. See `docs/selfchecks/player-quick-panel-selfcheck.md` and `docs/tech-decisions.md` #111.
