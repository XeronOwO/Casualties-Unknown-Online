...
### Review
- [Runtime entity creation can be absorbed by a markerless same-prefab copy](review/runtime-entity-markerless-bind-absorption.md) — **Low-Medium**: found by the round-4 re-review of E3; the markerless 1 m positional bind is DELETED — the enemy-domain backfill now stamps the creation key the host publishes with `EnemySnapshot.RuntimeSpawns` (`EnemySpawnEntryMsg.CreationKey`) and the trap-layout materialization stamps the key the host scans off its own marked copy (`TrapLayoutEntryMsg.CreationKey`), so every real copy carries identity and a markerless decoy can never absorb a record; two additive ProtoMembers, `ProtocolVersion` 17 → 18, red→green absorption tests, evidence anchors re-verified (793 entries); awaiting the final unified acceptance pass.
- [Dead runtime-entity relay API: `BroadcastEntitySpawned` has no caller](review/runtime-entity-dead-api-cleanup.md) — **Low**: found by the round-4 re-review of E3; the source-excluding relay method and its forwarding chain are DELETED (`rg` → zero hits). A source-excluding relay would be wrong here — the reporter's echo is the acknowledgement and carries the host's keypad code back. Live path unchanged; awaiting the final unified acceptance pass.
