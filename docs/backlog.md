...
- **Cross-player item use (give/feed/drink/wear/use an item on another player)** — **CLOSED (first slice + medicine + topical + opiates + component medicine + limb-tool + component-bearing tools + wear + shrapnel/timed tools, 2026-08-25)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) now covers drink/food, curated injectables, topical `ApplyToLimb`, opiates/antagonists, component medicine (`analgesicgauze` opiate component), non-liquid limb tools, component-bearing limb tools, native wearable placement, tweezers shrapnel removal, and medicalsuture timed bleed ticks. The additive `WornItem`/`TimedEffects` result fields carry the worn item and timed limb ticks to the target; no protocol bump. Remaining future slice: other timed/component medicine (timed/random liquid medicine branches). See `docs/tech-decisions.md` #96/#98/#100/#102/#103/#105/#106/#107/#108.
