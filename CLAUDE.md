...
- **Item physics: host-single simulation, complete-initial-condition reports, kinematic guest renders (landed 2026-08-09, user mandate; terminal form after the item-sync restructure)**: the host's physics is the ONLY simulation of any world item; guests' copies are kinematic renders driven by the host's 10 Hz unreliable position stream (EVERY standalone world item, sleeping included — filtering sleeping items made settled positions diverge again). One drop operation = ONE report carrying the COMPLETE initial vectors (position, linear velocity, angular velocity, rotation) — the game performs one drop as a DropItem→ThrowItem sequence plus a duplicate DropWearable hook, and reporting each call separately materialized a zero-velocity ghost whose wrong trajectory yanked the dropper's copy back ("dropped — immediately desynced"); the report side merges them (pending → ThrowItem consumes / next-frame flush). The 5 s keyframe reconciles only existence (kill missing / materialize new — never position, the stream owns that), guarded by the ~400 ms snapshot-race protection for freshly dropped/materialized items. NO "local grace period" mechanisms: letting the local copy simulate before following masks the start-point offset — the correct fix is completing the initial condition. Domain classes are one responsibility each: ItemWorldSync (reports + merge + race marking), ItemPositionAuthority (host pump), ItemPositionFollow (guest kinematic follow; targets expire when an item leaves the world domain), ItemApplication (materialize/bind/rollback + reconcile), DropProtectionGuard (race table — extracted so construction stays acyclic). The dead settle-report path was deleted outright.
