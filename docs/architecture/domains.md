...
|---|---|---|---|---|---|
| World / Run | `RunState` (`RunId`, `RandomState`, `BiomeOverride`, `BiomeDepth`, `TotalTraveled`, `LoadedRun`, `RunSettings`, `LayerIndex`, `LootRarityMultiplier`, `TrapRarityMultiplier`) | `StartRunCommand`, `AdvanceLayerCommand` | `RunStartedEvent`, `RunAdvancedEvent` | Run identity, seed, layer, global rules, epoch isolation | `Runtime/Session/World/WorldRunStateMapper.cs`, kernel checkpoint handoff through `KernelProtocolService` |
...
| Players | `PlayerStateTable` / `PlayerState` | `UpdatePlayerStatusCommand`, `SetPlayerCarryCommand`, `ClearPlayerCarryCommand`, `RecordPlayerInventoryTransferCommand`, `RecordPlayerHealResultCommand`, `RecordPlayerItemUseResultCommand` | `PlayerStatusUpdatedEvent`, `PlayerCarrySetEvent`, `PlayerCarryClearedEvent`, `PlayerInventoryTransferEvent`, `PlayerHealResultEvent`, `PlayerItemUseResultEvent` | Terminal health, body/limb latches, skills, carry relations, cross-player result facts — every one of them CROSS-LAYER, which is why this table has no layer-boundary reset at all (decision 174) | `PlayerKernelStatusProjection`, `PlayerKernelLimbProjection`, `PlayerKernelRestoreProjection`, `PlayerKernelCarryProjection`, `PlayerInteractionKernelProjection` |
...
| Entities / Enemies | `EnemyStateTable` / `EnemyState` | `UpsertEnemyCommand`, `RemoveEnemyCommand`, `ResetEnemiesCommand` (host-local, no wire form — decision 174), `RecordEnemyBiteCommand`, `RecordEnemyLungeCommand`, `RecordEnemyEffectCommand` | `EnemyUpsertedEvent`, `EnemyRemovedEvent`, `EnemiesResetEvent`, `EnemyBiteResultEvent`, `EnemyLungeResultEvent`, `EnemyEffectResultEvent` | Enemy lifecycle/health and combat terminal facts. The live rows are LAYER-SCOPED (the reset drops them at the boundary); the tombstones are terminal and survive it | `EnemyKernelProjection`, `EnemyKernelRestoreProjection`, `EnemyCombatKernelProjection` |
| Fluids | `FluidStateTable` / `FluidRegionState` | `UpdateFluidRegionCommand`, `ResetFluidsCommand` (host-local, no wire form — decision 174; issued at the generation boundary, while the stale chunks INSIDE a layer are still cleared by the zero-total upserts from `FluidKernelProjection`) | `FluidRegionUpdatedEvent`, `FluidsResetEvent` | Coarse authoritative region totals/checkpoints of ONE layer's grid, not every fluid pixel | `FluidKernelProjection`, `FluidKernelReadProjection` |
