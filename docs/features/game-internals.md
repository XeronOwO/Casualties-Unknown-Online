...
- `SaveSystem` writes `save.sv` (GZip JSON `SaveInfo`) and captures **character/run state only** — body/limb fields, carried/worn items and their `[Saveable]` components, recipes, run settings, `runTime`, `biome = biomeDepth + 1` (`reversing/Assembly-CSharp/Assembly-CSharp/SaveSystem.cs:35-187`). It carries **no world state** (no blocks, world items, entities, enemies or fluids); `WorldGeneration.ContinueRun()` regenerates the layer (`WorldGeneration.cs:1011-1020`) and `TryLoadGame` deletes the file after applying it (`SaveSystem.cs:196-455`, delete at `:453`). This is why a native save always restarts the current layer from its beginning (`ConsoleScript.cs:787-794` decrements `biomeDepth` before saving). `WorldSaveData` exists but is an unused two-field type (`WorldSaveData.cs:4-10`); `SaveSystem.loadedRun` / `ContinueRun` drive the resume flow.
...
- `Body.Update` → `HandleVisuals` (`Body.cs:3123+`) — drives limb poses from the Animator skeleton (`bodyAnimator`/`armsAnimator`) + local world queries (BoxCast for `grounded`); also `Random.Range` jitter (consumes RNG). **MUST run on proxies** or limb sprites stay uninitialized/invisible and no poses are driven. For ragdoll/dead/unconscious the 20 Hz stream can carry exact owner limb poses; while active, the proxy suppresses the animator-to-visible copy so the real poses are not overwritten.
...
Each player simulates **only its own body** locally; peer state is exchanged at 20 Hz through `StateStreamEnvelope` over `KernelEnvelope` (host→guest and guest→host player reports; both carry position/look/velocity/pose flags, plus per-limb **world-space** position/rotation while the owner is not standing and not sleeping). The remote player is a frozen render clone fed by the state stream. "Host-authoritative" does **not** mean the host computes the guest's movement — guest movement is always local (user mandate: "移动必定是在本地计算的,主机只做校验"); authority covers world-state ownership (world-gen seed, saves, later: interactions), not per-frame player simulation. Previous attempt (host shadow-simulating the guest's clone) was reverted (`882a43d`).
