...
- [Global adaptive report-rate / sync-frequency flow control](todo/global-adaptive-report-rate-flow-control.md) — **Medium (user-promoted from future)**: global framework for all frequent-report, loss-tolerant streams; split into stages; Stages 1-2 are in review and the remaining stage tickets are created as sessions reach them.
