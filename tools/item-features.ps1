...
.DESCRIPTION
    ...
    read validates the table first; every write validates after — a
    misaligned row aborts with exit 1, never silently. The CSV is UTF-8
    without BOM (git-clean); cells with commas must be quoted (Import-Csv /
    ConvertTo-Csv round-trip quotes automatically).

    Commands:
      validate                        column alignment + item uniqueness + column names
      list                            all data rows, tab-separated
      get <item> [feature]            one item's row / one cell
      set <item> <feature> <value>    edit one cell (adds the row if missing)
      add-item <item> [feature=value...]  append a row (blank cells unless given)
...
.EXAMPLE
    ...
    tools/item-features.ps1 list
    tools/item-features.ps1 get flashlight modeswitch
    tools/item-features.ps1 set flashlight modeswitch "0-3"
    tools/item-features.ps1 add-item newitem battery=yes
    tools/item-features.ps1 add-feature newfeature
#>
param(
    [Parameter(Position = 0, Mandatory = $true)]
    [ValidateSet('validate', 'list', 'get', 'set', 'add-item', 'remove-item', 'add-feature')]
    [string]$Command,
...
function Assert-Valid {
    ...
    if ($headerCount -lt 2) { throw "Header row needs at least the '$itemColumn' column plus one feature column." }
    if ($header[0] -ne $itemColumn) { throw "First header cell must be '$itemColumn' (found '$($header[0])')." }
    ...
    for ($i = 1; $i -lt $lines.Count; $i++) {
        ...
        $item = $cells[0]
        if ($seen.ContainsKey($item)) { throw "Duplicate item '$item' on row $($i + 1)." }
        $seen[$item] = $true
    }
...
# Feature columns = every column except the item column.
...
switch ($Command) {
    ...
    'get' {
        if ([string]::IsNullOrEmpty($Arg1)) { Write-Error "get needs <item> [feature]."; exit 1 }
        try { Assert-Valid | Out-Null }
        catch { Write-Error $_.Exception.Message; exit 1 }
        $row = Read-Matrix | Where-Object { $_.$itemColumn -eq $Arg1 }
        if (-not $row) { Write-Error "Item '$Arg1' not in matrix."; exit 1 }
    ...
    'set' {
        if ([string]::IsNullOrEmpty($Arg1) -or [string]::IsNullOrEmpty($Arg2)) { Write-Error "set needs <item> <feature> <value>."; exit 1 }
        try { $header = Assert-Valid }
        catch { Write-Error $_.Exception.Message; exit 1 }
        if ($header -notcontains $Arg2) { Write-Error "Feature '$Arg2' not a matrix column."; exit 1 }
        $rows = Read-Matrix
        $row = $rows | Where-Object { $_.$itemColumn -eq $Arg1 }
        if (-not $row) {
            $new = [PSCustomObject]@{}
    ...
    'add-item' {
        if ([string]::IsNullOrEmpty($Arg1)) { Write-Error "add-item needs <item> [feature=value ...]."; exit 1 }
        try { $header = Assert-Valid }
        catch { Write-Error $_.Exception.Message; exit 1 }
        $rows = Read-Matrix
        if ($rows | Where-Object { $_.$itemColumn -eq $Arg1 }) { Write-Error "Item '$Arg1' already in matrix."; exit 1 }

        $new = [PSCustomObject]@{}
        foreach ($h in $header) { $new | Add-Member -NotePropertyName $h -NotePropertyValue '' }
        $new.$itemColumn = $Arg1
        # feature=value pairs arrive as the positional args after the item
        ...
        Write-Host "add-item: $Arg1"
        exit 0
    }

    'remove-item' {
        if ([string]::IsNullOrEmpty($Arg1)) { Write-Error "remove-item needs <item>."; exit 1 }
        try { Assert-Valid | Out-Null }
        catch { Write-Error $_.Exception.Message; exit 1 }
        $rows = Read-Matrix
        $kept = @($rows | Where-Object { $_.$itemColumn -ne $Arg1 })
        if ($kept.Count -eq $rows.Count) { Write-Error "Item '$Arg1' not in matrix."; exit 1 }
        Write-Matrix $kept
        try { Assert-Valid | Out-Null }
        catch { Write-Error "Write left the matrix invalid: $($_.Exception.Message)"; exit 1 }
        Write-Host "remove-item: $Arg1"
        exit 0
    }
