...
| Current phase status | In progress — Phase E inventory, dead-seam removal, shadow terminology cleanup, test-only helper relocation, session reset audit, no-legacy/command-authority/kernel-shape guards, and several facade-narrowing batches are landed; generic Prediction Runtime remains a Phase E residue item (tech-decisions.md #157) |
