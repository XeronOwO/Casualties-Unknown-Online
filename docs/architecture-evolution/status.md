...
| E — Delete dual architecture | Completed | 2026-08-30 | Legacy inventory, dead-seam removals, centralized kernel reset, facade narrowing, no-legacy/command-authority/kernel-shape guards, and `ItemReject` → KernelEnvelope migration are landed. See `docs/selfchecks/phase-e-legacy-inventory-selfcheck.md` and `docs/tech-decisions.md` #158. |
...
| 2026-08-30 | E — Delete dual architecture | See current commit | 1791 tests green; build/format/architecture (strict + no-legacy/command-authority/kernel-shape)/event/entity/delivery gates pass; `ItemReject` migrated to `KernelEnvelope` `CommandRejected`; session reset centralized; direct `NetMsg` families classified as active single-path. See `docs/selfchecks/phase-e-legacy-inventory-selfcheck.md` and `docs/tech-decisions.md` #158. | Architecture evolution complete; future work treats the typed kernel as the only supported design. |
