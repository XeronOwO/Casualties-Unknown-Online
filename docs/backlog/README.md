...
- [Runtime-created BuildingEntity spawns have no backfill or re-report](todo/runtime-entity-spawn-backfill.md) — **Medium-High**: audit gap E3; `EntitySpawned` is one-shot, the world-entry group and the kernel checkpoint carry no creation table, so a swallowed report leaves a permanently one-sided entity until reconnect.
