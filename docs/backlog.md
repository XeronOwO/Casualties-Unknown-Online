...
- **Large logical class debt flattening** — HIGH (follow-up to the closed gate, in progress). The gate now sees the remaining recorded aggregate debt: `ModService` 1590, `GameAdapter` 1397, `ItemService` 928, `WorldService` 899. `PlayerInteractionService` (716), `ItemApplication` (630) and `EnemySyncCoordinator` (750) were flattened this cycle (2026-08-23): the first split into real top-level `PlayerInventoryTakeService` / `PlayerCarryService` / `PlayerHealService` + shared `PlayerCharacterAccess`; the second's cook-replay half extracted into `ItemCookReplayApplier`; the third's host-ordered attack/bite replay extracted into `EnemyCombatReplay` (aggregate 750 → 542). All three were removed from the debt ledger. Remaining classes must still be split into real top-level responsibilities, not more partial files. See `docs/architecture-debt.json`, `docs/selfchecks/player-interaction-service-split-selfcheck.md`, `docs/selfchecks/item-cook-replay-split-selfcheck.md`, `docs/selfchecks/enemy-combat-replay-split-selfcheck.md`, `docs/tech-decisions.md` #66/#67/#68 and §3.1.
