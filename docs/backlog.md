...
- **Large logical class debt flattening** — HIGH (follow-up to the closed gate, in progress). The gate now sees the remaining recorded aggregate debt: `ModService` 1590, `GameAdapter` 1397, `ItemService` 928. `PlayerInteractionService` (716), `ItemApplication` (630), `EnemySyncCoordinator` (750) and `WorldService` (899) were flattened this cycle (2026-08-23): the first split into real top-level `PlayerInventoryTakeService` / `PlayerCarryService` / `PlayerHealService` + shared `PlayerCharacterAccess`; the second's cook-replay half extracted into `ItemCookReplayApplier`; the third's host-ordered attack/bite replay extracted into `EnemyCombatReplay`; the fourth's message-flow and channel-forwarding halves extracted into `WorldStateMessageService` + `WorldChannelRelay` (aggregate 899 → 423 facade). All four were removed from the debt ledger. Remaining classes must still be split into real top-level responsibilities, not more partial files. See `docs/architecture-debt.json`, `docs/selfchecks/player-interaction-service-split-selfcheck.md`, `docs/selfchecks/item-cook-replay-split-selfcheck.md`, `docs/selfchecks/enemy-combat-replay-split-selfcheck.md`, `docs/selfchecks/world-service-split-selfcheck.md`, `docs/tech-decisions.md` #66/#67/#68/#69 and §3.1.
...
`ItemService.cs` was split into message-flow partials in the 2026-08-22 architecture cycle (each main file + new partial is under the gate; see `docs/selfchecks/world-item-service-partial-split-selfcheck.md`). `WorldService.cs` was further flattened in this cycle into `WorldStateMessageService` + `WorldChannelRelay` and removed from the debt ledger.
