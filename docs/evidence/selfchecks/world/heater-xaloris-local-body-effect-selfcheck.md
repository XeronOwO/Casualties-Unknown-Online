...
| 2 | `Heater` on `xaloris` | `docs/features/enemies.md` runtime-verified prefab mapping: `xaloris` carries `XalorisScript` + `Heater`; the guest copy is frozen only for AI/physics (EnemyPatches skips `Update`/`FixedUpdate`), not for local-body field components |
