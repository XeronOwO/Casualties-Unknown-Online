...
public class GameFieldContractTests
{
	...
	[
		...
		("CrystalEnemy", "timeBeforeAttack", Kind.Field, typeof(float), "the wind-up countdown — read as float (CrystalWindupPresentation.CaptureAmount)"),
		("CrystalEnemy", "rend", Kind.Field, "UnityEngine.LineRenderer", "the telegraph renderer — read as LineRenderer (CrystalWindupPresentation)"),
		("CrystalMetamorphic", "activated", Kind.Field, typeof(bool), "the touch latch (TrapCrystalPatch.MetamorphicTouchedPatch)"),
		("CrystalShy", "activated", Kind.Field, typeof(bool), "the touch latch (TrapCrystalPatch.ShyTouchedPatch, mirrored by CrystalStateActions.ApplyCrystalShy through CrystalEffectAccess)"),
		("CrystalEMP", "activated", Kind.Field, typeof(bool), "the TryEMP latch (TrapCrystalPatch.EmpTryEMPPatch, mirrored by CrystalStateActions.ApplyCrystalEMP — the crystal's own Update returns unless it is set, CrystalEMP.cs:54-58)"),
		...
		("CrystalMetamorphic", "activated", Kind.Field, typeof(bool), "the metamorphic one-shot latch — the guard its own Touched checks before rolling its 1..4 drops (CrystalMetamorphic.cs:18-21/:27-33), mirrored by CrystalStateActions.ApplyCrystalMetamorphic"),
		("CrystalBehaviour", "effects", Kind.Field, null, "the crystal effect list — read UNTYPED as IEnumerable by CrystalEffectAccess.Find, which serves every per-effect accessor; the adapter only needs an effect's runtime type name + its activated field"),
		// The trader domain.
		...
		("RadiationLine", "timeGone", Kind.Field, typeof(float), "the radiation line's authoritative descent (RadiationLineSync reads via Traverse — exact float)"),
