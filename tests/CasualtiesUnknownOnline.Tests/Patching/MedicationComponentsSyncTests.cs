...
/// <summary>
/// Reflective surface for the Game Adapter's medication component capture/apply
/// helper. SleepingPills/Antidepressants/MindwipeScript live on components, not
/// on <c>Body</c>, so they cannot ride Mapster; this contract locks the helper
...
public sealed class MedicationComponentsSyncTests
{
	private static readonly Type Sync = GameAssemblyHost.Adapter.GetType(
		"CasualtiesUnknownOnline.GameAdapter.Character.MedicationComponentsSync",
		throwOnError: true)!;

	[Fact]
	public void Surface_HasCaptureAndApplyForMedicationComponents()
	{
		var capture = Sync.GetMethod("Capture", BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public)
			?? throw new InvalidOperationException("MedicationComponentsSync.Capture not found.");
		var apply = Sync.GetMethod("Apply", BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public)
			?? throw new InvalidOperationException("MedicationComponentsSync.Apply not found.");
