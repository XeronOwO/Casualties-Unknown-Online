...
| D — Full domain migration | In progress (all domain areas have kernel foundations) | 2026-08-29 | World/Run/Epoch and WorldEntities authority switches complete. Players terminal-status, body-level terminal latches, limb latches and carry relations, Enemy/Entity lifecycle-health, and Fluids region-checkpoint kernel baselines landed; the carry wire is fully cut over to kernel committed batches; cross-player take/heal/use now sync host-recipient and wear-to-host item ownership/state into the item kernel and guest replay kernel receives the same facts (legacy `NetMsg.PlayerCarryState` + handler removed, host/guest carry mirror projection and checkpoint rebuild covered); player stream lifecycle audit (update-only + explicit join/leave) is covered; Fluids host grid derives and commits the coarse kernel checkpoint; enemy 20 Hz stream is update-only with explicit reliable removal, a session-scoped resurrection guard, and a terminal revision guard preventing stale stream rollback of kernel health; guest WorldEntities checkpoint projection landed and the legacy WorldEntities snapshot wire was removed. 1697 tests green. See the phase-d selfcheck set in `docs/selfchecks/`. |
