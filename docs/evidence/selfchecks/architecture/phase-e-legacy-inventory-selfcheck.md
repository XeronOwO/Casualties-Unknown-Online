...
| 2026-08-30 | Narrow session-reset surface | `ItemService.cs`, `WorldService.cs`, `docs/history/architecture-blueprint.md` | `dotnet build` 0 warnings/0 errors; 1792 tests passed; format + architecture/event/entity/delivery gates passed |
...
| 2026-08-30 | Classify all remaining direct `NetMsg` families | `docs/evidence/selfchecks/architecture/phase-e-legacy-inventory-selfcheck.md` | Docs | Every active direct frame is now recorded as session/control, presentation, request, or non-kernel-domain path, not kernel-dual authority |
