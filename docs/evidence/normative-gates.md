...
| The protocol version number lives in ONE place: `ProtocolVersion.Current` and its doc comment are the value and the wire-change log, so a live governance document points at the constant instead of restating it — the fixed list is root `AGENTS.md`, `docs/README.md`, `docs/decisions/active.md`, `docs/architecture/{README,current,protocol,domains}.md`, `docs/development/agent-reference.md`, `docs/evidence/normative-gates.md` and the six mod-facing pages `docs/en|zh/reference/{mod-api,modification-policy,protocol-messages}.md`, and the other pages under `docs/architecture/` are NOT scanned — while records of a past state (`docs/evidence/**`, backlog tickets, the evolution logs) keep their own historical number on purpose | dotnet test (C# port) | `ProtocolNumberGateTests.LiveGovernanceDocuments_DoNotRestateTheProtocolVersionNumber` (a census floor at the array's own length plus a check that the constant still exists, and every fixed path existence-checked) + `...TheMatcher_FlagsACurrentValueClaimAndIgnoresThePointerForm` |
...
|---|---|---|
| A document is a layer, not just a file: an agent document answers how to do the work and stays English only, a human page answers what the product is and is paired English + Chinese, and a process record records what a cycle did — English, in place, outside the human navigation, its conclusions absorbed into the pages. The rules for the two blocks are `docs/AGENTS.md`, the renderings are `docs/standard/terminology.txt`, and the instruction chain that routes an agent into them (root `AGENTS.md`, `docs/AGENTS.md`, the block files and the machine-local file, which all compete for ONE loader budget) stays inside it | Review / process + dotnet test (C# port) | The policy half is review-checked against `docs/AGENTS.md`; the mechanical half is `DocumentationTreeGateTests` (row below) and `AgentInstructionBudgetGateTests` — `EveryNestedInstructionFile_StaysUnderItsCeiling`, `TheWholeChain_StaysInsideTheWorkspaceBudget` (the machine-local file included, because it competes for the same budget) and `TheCeilingCheck_FlagsExactlyTheFilesOverTheLimit` |
| Human page trees (`docs/AGENTS.md` §1–§7): the two mirrored blocks hold the same page set at the same paths, every relative link resolves, every content page carries the fixed shape, an index page carries its breadcrumb and names its own directory, the Chinese pages use Chinese punctuation in prose, and every pair is recorded in `docs/standard/alignment.txt` at its current contents | dotnet test (C# port) | `DocumentationTreeGateTests` — `EveryPage_HasItsCounterpartInTheOtherBlock`, `EveryRelativeLink_PointsAtAFileThatExists`, `EveryPage_CarriesTheDocumentShape`, `EveryPair_IsRecordedAtItsCurrentContents`, `EveryIndex_NamesItsOwnDirectory`, `EveryChinesePage_UsesChinesePunctuation`, each with its own matcher contract |
| Translation quality — whether a counterpart says the same thing accurately, well-termed and naturally | Review / process | The gate compares page parity, link targets and blob hashes, never meaning: a re-recorded pair with a sloppy counterpart passes the gate and must not pass review (the limit is stated in `docs/AGENTS.md` §6 and `docs/en/contributing/documentation-standard.md`). |
| The two feature matrices are read through `tools/item-features.ps1` and `tools/entity-features.ps1`, and each tool resolves its own CSV through a `Join-Path $scriptDir` literal that must name a file that exists; each matrix's feature columns and the column list the two `reference/feature-matrices.md` pages publish are one list, so a matrix move, a tool that stops resolving its own matrix or a renamed column fails until tool, CSV and both pages agree | dotnet test (C# port) | `FeatureMatrixToolPathGateTests` — `EveryScriptRelativeLiteral_NamesAFileThatExistsAndResolvesItsOwnMatrix`, `TheMatrixHeaders_EqualTheFeatureColumnListsTheReferencePagesPublish`, each with its matcher contract (`TheLiteralMatcher_ReadsTheJoinPathFormAndIgnoresTheOtherForms`, `ThePageColumnMatcher_ReadsOnlyTheBacktickedFirstColumn`) |
...
- `tests/CasualtiesUnknownOnline.NormativeGates.Tests/DocumentationTreeGateTests.cs`
- `tests/CasualtiesUnknownOnline.NormativeGates.Tests/AgentInstructionBudgetGateTests.cs`
- `tests/CasualtiesUnknownOnline.Tests/World/EntityFeaturesDocConsistencyTests.cs`
