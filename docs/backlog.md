...
- **Cross-player item use (give/feed/drink/wear/use an item on another player)** — **CLOSED (first slice + medicine + topical + opiates + limb-tool + component-bearing tools, 2026-08-25)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) covers drink/food, curated immediate-effect injectables, topical `ApplyToLimb` containers, curated injectable opiates/opiate-antagonists (`morphine`/`opium`/`heroin`/`fentanyl`/`naloxone`), the curated non-liquid limb-tool set (`boneweldingtool`/`clottingmush`/`chestdrain`/`musharm`), and the component-bearing limb-tool set (`splint`/`carcasssplint`/`tourniquet`/`icepack`); no second wire operation. The opiate `Painkillers` component state rides `CharacterHealthMsg`, and limb `[Saveable]` component state now rides `CharacterLimbMsg.Components` (additive fields, no protocol bump). Wear, timed/component medicine, minigame-random tools, and timed tools remain future slices. See `docs/selfchecks/cross-player-item-use-selfcheck.md`, `docs/selfchecks/cross-player-medicine-use-selfcheck.md`, `docs/selfchecks/cross-player-topical-use-selfcheck.md`, `docs/selfchecks/cross-player-opiate-use-selfcheck.md`, `docs/selfchecks/cross-player-limb-tool-use-selfcheck.md`, `docs/selfchecks/cross-player-component-tool-use-selfcheck.md`, and `docs/tech-decisions.md` #96/#98/#100/#102/#103/#105.
