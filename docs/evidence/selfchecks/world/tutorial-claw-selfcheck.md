...
| 6 | Call identity, not patch state | `TutorialHandlerUpdatePatch` opens `CallContext.Origin.TutorialClawSpawn` for the duration of the original `Update` and disposes it in the postfix (`__state` only — no cross-call patch state, the sync-chain rule in `docs/development/agent-reference.md`). `UtilsCreateTutorialPatch` marks only `Utils.Create` results inside that scope |
...
- Touched classes stay under the 600-line gate: `TutorialClawProp` 21,
  ...
  — `ItemWorldSync` 411, `ItemApplication` 576, `EntitySpawnSync` 299, `CallContext` 93 (architecture gate passed).
