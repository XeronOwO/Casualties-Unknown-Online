...
- **Dedicated standalone player-interaction UI** — **CLOSED (2026-08-25)**. A hotkey-docked `OnlineUiQuickPanel` (default `[Session] InteractionPanelKey=F6`) shows the selected in-world remote player's status/inventory and all eligible co-op actions outside the full Online window; the target auto-follows the current/nearest remote and a compact selector handles multiple remotes. The in-world right-click context menu remains the transient cursor-based entry. No wire/protocol change. See `docs/selfchecks/player-quick-panel-selfcheck.md` and `docs/tech-decisions.md` #111.
