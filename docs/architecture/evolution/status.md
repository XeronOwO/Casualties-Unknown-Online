...
architecture is described in [current-architecture.md](../current.md),
...
| Current phase status | Completed — Phase E complete; the legacy `ItemReject` frame was migrated to `KernelEnvelope` `CommandRejected`, all migrated Phase D domains run on the typed kernel, and the Phase E guard suite is green |
| Last status update | 2026-08-30 |
| Next work session | Architecture evolution complete. Future feature work should treat the typed kernel as the only supported design; keep the Phase E guard suite green and track any future Prediction Runtime/gameplay work separately. |
| Protocol/save compatibility | The new four-envelope protocol and checkpoint save stack are the production item paths. Old item packet handlers/DTOs and the corresponding `NetMsg` item enums have been fully removed; block-break drop refusal now rides `KernelEnvelope` `CommandRejected`. World/Run, WorldEntities, Players (status, limb latches, body latches, skills, carry), Enemy, and Fluids kernel/checkpoint baselines now exist; WorldEntities world-entry backfill rides the kernel checkpoint, carry state now rides `KernelEnvelope` committed batches (`NetMsg.PlayerCarryState` removed), enemy combat results (bite/lunge/proximity) now ride journal-only kernel events (`NetMsg.EnemyBite`/`EnemyLunge`/`EnemyEffect` removed), and enemy aggregate removal rides `EnemyRemovedEvent` (`NetMsg.EnemyRemoved` removed). |
...
| E — Delete dual architecture | Completed | 2026-08-30 | Legacy inventory, dead-seam removals, centralized kernel reset, facade narrowing, no-legacy/command-authority/kernel-shape guards, and `ItemReject` → KernelEnvelope migration are landed. See `docs/evidence/selfchecks/architecture/phase-e-legacy-inventory-selfcheck.md` and `docs/decisions/active.md` #158. |
...
| 2026-08-30 | D — Full domain migration | See current commit | 1794 tests green before closure; all Phase D checklist items resolved; 4.3 prediction/rollback boundary closed by deferring the generic Prediction Runtime to Phase E (tech-decisions.md #157); per-domain selfchecks plus `docs/evidence/selfchecks/architecture/phase-d-full-domain-migration-selfcheck.md` | Phase E delete dual architecture is next; start with the legacy inventory including ad-hoc prediction/rollback caches. |
| 2026-08-30 | E — Delete dual architecture | See current commit | 1791 tests green; build/format/architecture (strict + no-legacy/command-authority/kernel-shape)/event/entity/delivery gates pass; `ItemReject` migrated to `KernelEnvelope` `CommandRejected`; session reset centralized; direct `NetMsg` families classified as active single-path. See `docs/evidence/selfchecks/architecture/phase-e-legacy-inventory-selfcheck.md` and `docs/decisions/active.md` #158. | Architecture evolution complete; future work treats the typed kernel as the only supported design. |
...
3. Relevant decisions are recorded in `docs/decisions/active.md`.
