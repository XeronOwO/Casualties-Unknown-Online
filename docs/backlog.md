...
- **Cross-player item use (give/feed/drink/wear/use an item on another player)** — **CLOSED (first slice, 2026-08-24)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) lets a player use a carried drink/food on an in-world teammate; `RemoteConsumeCatalog` + `RemoteConsumeApplication` cover curated liquids/solid foods, and the Players page + in-world context menu expose Use/Use-with-item. Wear/injectables/timed medicine/tools remain future slices. See `docs/selfchecks/cross-player-item-use-selfcheck.md` and `docs/tech-decisions.md` #96.
