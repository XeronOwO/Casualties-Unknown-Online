...
| Current phase status | In progress — all six Phase D domain areas have kernel foundations; Fluids host region kernel production wiring landed |
| Last status update | 2026-08-29 |
| Next work session | Continue Phase D: high-frequency state stream unification, world-entity snapshot cleanup, and any remaining guest-side fluid projection. |
...
| D — Full domain migration | In progress (all domain areas have kernel foundations) | 2026-08-29 | World/Run/Epoch and WorldEntities authority switches complete. Players terminal-status, Enemy/Entity lifecycle-health, and Fluids region-checkpoint kernel baselines landed; Fluids host grid now derives and commits the coarse kernel region checkpoint. 1670 tests green. See the phase-d selfcheck set in `docs/selfchecks/`. |
