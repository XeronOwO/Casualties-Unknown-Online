...
- [Guest world-block mutations have no periodic re-report](todo/guest-block-mutation-re-report.md) — **High**: audit gap W1/W2; the host's 60 s absolute table cannot heal a swallowed guest report because the guest keeps no diff table and never re-reports; partial block damage shares the asymmetry.
