...
public class ItemKernelShadowTests
{
	...
	public void ObserveLifecycle_DrivesKernelState()
	{
		...
		var spawned = authority.KernelForDiagnostics.FindItem(42)!.Value;
		Assert.Equal(ItemLocationKind.World, spawned.Location.Kind);

		authority.ObservePickup(2001, 42);
		var carried = authority.KernelForDiagnostics.FindItem(42)!.Value;
		...
		var dropped = authority.KernelForDiagnostics.FindItem(42)!.Value;
		...
		var terminal = authority.KernelForDiagnostics.FindItem(42)!.Value;
	...
	public void ResetForSession_StartsFreshEpoch()
	{
		var authority = new ItemKernelAuthority(NullLogger<ItemKernelAuthority>.Instance);
		authority.ObserveSpawn(1001, 42, "test_item", 1, 2);
		Assert.NotNull(authority.KernelForDiagnostics.FindItem(42));

		authority.ResetForSession();

		Assert.Null(authority.KernelForDiagnostics.FindItem(42));
	}
}
