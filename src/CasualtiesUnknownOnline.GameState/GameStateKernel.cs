...
public sealed class GameStateKernel(RunEpoch runEpoch) : IGameStateKernel
{
	...
	public Decision Execute(GameCommand command, CommandContext context)
	{
		...
		var decision = module.Decide(command, new KernelReadModel(_store.RunEpoch, _store.GlobalRevision, _store.Items, _store.Run, _store.WorldEntities, _store.Players, _store.Enemies, _store.Fluids), context);
		...
		try
		{
			module.AssertInvariants(new KernelReadModel(working.RunEpoch, working.GlobalRevision, working.Items, working.Run, working.WorldEntities, working.Players, working.Enemies, working.Fluids));
		}
	...
	public FluidStateTable? QueryFluids() => _store.Fluids;
}
