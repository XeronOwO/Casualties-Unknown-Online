...
- `validate -Doc docs/features/items.md` additionally warns (not fails) when a
...
Randomness: see [r[andomaction](#randomaction—random-behavior-out-of-sync-by-design).
...
[c[onsumable](#consumable—eatdrinkinject)) so the mode switch is never
corrected back.
...
`docs/evidence/selfchecks/items/dynamite-explosion-selfcheck.md`,
`docs/evidence/selfchecks/items/custom-item-data-state-selfcheck.md` and
`docs/evidence/selfchecks/items/dynamite-fuse-presentation-selfcheck.md`.
...
203) and spread `Random.Range` (213) — see [r[andomaction](#randomaction—random-behavior-out-of-sync-by-design).
...
`docs/evidence/selfchecks/presentation/muzzle-flash-sync-selfcheck.md`.
...
relays the whole report (source excluded). Details in docs/decisions/active.md ("Crafting
...
- **CustomItemBehaviour.data** — see
  [p[ayload](#payload—customitembbehaviourdata); the liquidcentrifuge
...
- **World-item component state on keyframes**: RESOLVED (2026-08-21, no
  ...
  `docs/evidence/selfchecks/items/item-keyframe-state-selfcheck.md`.
...
`docs/history/audits/runtime-supply-refresh-audit.md`.
