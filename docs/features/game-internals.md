...
Each player simulates **only its own body** locally; peer state is exchanged at 20 Hz through `StateStreamEnvelope` over `KernelEnvelope` (host→guest and guest→host player reports; both carry position/look/velocity/pose flags, plus per-limb local position/rotation while the owner is not standing and not sleeping). The remote player is a frozen render clone fed by the state stream. "Host-authoritative" does **not** mean the host computes the guest's movement — guest movement is always local (user mandate: "移动必定是在本地计算的,主机只做校验"); authority covers world-state ownership (world-gen seed, saves, later: interactions), not per-frame player simulation. Previous attempt (host shadow-simulating the guest's clone) was reverted (`882a43d`).
