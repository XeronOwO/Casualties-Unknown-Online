...
|---|---|---|---|---|
| 1 | Game-facing probe | `GameAdapter.ProbeGame()` is four `typeof(...)` reads tested with `is not null`, which cannot fail for a compile-time reference — effectively a constant true | untouched (that is `review/adapter-capability-catalog.md`'s subject); the toolchain reads the assembly instead of trusting a probe | `src/CasualtiesUnknownOnline.GameAdapter/GameAdapter.cs` (`public bool ProbeGame()`, the four `typeof` reads) |
