...
2. **Phase 1 — Single player entity**: join/leave, position, heading, basic input, scene load state. Sync model landed (2026-08-06): **each player simulates ONLY its own body locally**; peer state exchanged at 20 Hz (`PlayerState` host→guest, `PlayerStateReport` guest→host); the remote player is a frozen render clone (physics off, animations on) fed by the state stream. "Host-authoritative" does NOT mean the host computes the guest's movement — guest movement is always local (user mandate); authority covers world-state ownership (world-gen seed, saves, later interactions), never per-frame player simulation. Details in `docs/game-internals.md` §Sync Model.
