...
| 5 | GrabberPlant grab | Existing no-grab→grab edge detection now reports `EnemyEffectKind.GrabberGrabbed` with shock/eye-panic terminal state; `EntityEventKind.GrabberGrabbed` becomes a layout-only key | `GrabberPlant.cs:75-90`; `TrapGrabberPlantPatch`; `docs/event-replay-matrix.csv` row update |
