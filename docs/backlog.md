...
- **Large logical class debt flattening** — HIGH (follow-up to the closed gate, in progress). The gate now sees the remaining recorded aggregate debt: `ModService` 1590, `GameAdapter` 1397. `PlayerInteractionService` (716), `ItemApplication` (630), `EnemySyncCoordinator` (750), `WorldService` (899) and `ItemService` (928) were flattened this cycle (2026-08-23): the first split into real top-level `PlayerInventoryTakeService` / `PlayerCarryService` / `PlayerHealService` + shared `PlayerCharacterAccess`; the second's cook-replay half extracted into `ItemCookReplayApplier`; the third's host-ordered attack/bite replay extracted into `EnemyCombatReplay`; the fourth's message-flow and channel-forwarding halves extracted into `WorldStateMessageService` + `WorldChannelRelay`; the fifth's report/receive flow and pending-pickup arbitration extracted into `ItemMessageFlowService` + `ItemPendingPickupArbiter`. **`ModService` (1590) is now CLOSED (2026-08-23)**: the facade is a 98-line normal class, with real top-level `ModLifecycle`, `ModCommandService`, `ModStateStore`, `ModContext` plus `ModCatalog` / `ModPermissionGate` / `ModSessionSnapshot`; all seven old partials were deleted. **Only `GameAdapter` (1397) remains recorded.** Remaining classes must still be split into real top-level responsibilities, not more partial files. See `docs/architecture-debt.json`, `docs/selfchecks/player-interaction-service-split-selfcheck.md`, `docs/selfchecks/item-cook-replay-split-selfcheck.md`, `docs/selfchecks/enemy-combat-replay-split-selfcheck.md`, `docs/selfchecks/world-service-split-selfcheck.md`, `docs/selfchecks/item-service-split-selfcheck.md`, `docs/selfchecks/mod-service-split-selfcheck.md`, `docs/tech-decisions.md` #66/#67/#68/#69/#70/#71 and §3.1.
...
- **`HandlerContext` god-object** — MEDIUM (partially addressed). It still injects many control planes; the world-entry fan-out half is done — it now lives in `WorldEntryFanout` (see §3.4), so `HandlerContext` no longer owns a concrete world-entry flow. Remaining: narrow per-domain handler dependencies. See §3.3.
...
`SessionService.cs` (580), `ItemApplication.cs` (576), `CharacterDataSync.cs` (563), `EntitySyncService.cs` (548), `EnemyCombatDirector.cs` (547), `Plugin.cs` (534), `RunCoordinator.cs` (512).
