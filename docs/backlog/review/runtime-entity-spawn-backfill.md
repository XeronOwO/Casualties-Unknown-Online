...
| Channel | animal recording by key; snapshot ack list; a creation the host cannot materialize is REJECTED and answered (RuntimeEntityRejected) instead of relayed; key-based death; tokenless warning | `RuntimeEntityChannel.cs`; `GuestEntityReportRecoveryTests` (16); `RuntimeEntityRejectionTests` (6) |
