...
| Drops / AnimalDeath | Drops + experience reward unchanged (attacker-side only); remote side now replays the creature-specific death presentation for live remote deaths | `BuildingEntity.cs:75-121`; `RemoteEntityDeath`; `AnimalDeathReplay.cs`; `docs/evidence/selfchecks/enemies/animal-death-presentation-selfcheck.md` |
