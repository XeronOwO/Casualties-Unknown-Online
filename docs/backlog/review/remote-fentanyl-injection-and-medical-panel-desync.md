...
- **Remote display fidelity**: the inactive display body's heart rate, blood pressure readout, opiate happiness, antidepressant happiness and mindwipe are projected; stale mindwipe components are removed when no longer present. This path has since been absorbed into `RemoteCharacterDisplayProjection` by the unified remote display projection rework.
