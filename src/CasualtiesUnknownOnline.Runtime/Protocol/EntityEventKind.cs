...
public enum EntityEventKind : byte
{
	...
	CrystalMetamorphicTriggered = 27, // metamorphic crystal: touched → FlashBrief + health = 0 + 1..4 drops + laugh (CrystalMetamorphic.cs:16-35); the DROPS are the trigger side's item-domain fact and the entity half (the effect's activated latch + the death as a remote death) is mirrored by the replay — the kind is deliberately NOT in TrapDamageProfiles, so no destroyed-health row rides the trigger batch — one-shot
