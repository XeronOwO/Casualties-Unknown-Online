...
- **Large logical class debt flattening** — HIGH (follow-up to the closed gate). The gate now sees the remaining recorded aggregate debt: `ModService` 1590, `GameAdapter` 1397, `ItemService` 928, `WorldService` 899, `EnemySyncCoordinator` 750, `PlayerInteractionService` 716, `ItemApplication` 630. Each must be split into real top-level responsibilities, not more partial files. See `docs/architecture-debt.json` and §3.1.
