...
| Current phase status | In progress — all six Phase D domain areas have kernel foundations; Fluids host production wiring landed; enemy stream lifecycle slice landed; Players carry kernel shadow + production projection, limb terminal facts, and body-level terminal latches landed; WorldEntities guest checkpoint projection landed and the legacy world-entity snapshot wire removed |
