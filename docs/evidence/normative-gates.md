...
| Every synced feature declares its event-sync + periodic-fallback policy: every `NetMsg` / `WireCommandKind` / `WireEventKind` / `AdaptiveStreamId` member is indexed in the sync coverage matrix, the index points at a row that mentions it, and every evidence entry still matches the source line it quotes | dotnet test (C# port) | `SyncCoverageGateTests.SyncCoverageMatrix_EveryWireDiscriminatorIsIndexed` + `SyncCoverageGateTests.SyncCoverageMatrix_EveryRowHasAVerdictAndEvidence` + `SyncCoverageGateTests.SyncCoverageEvidence_EveryQuoteMatchesItsSourceLine` (negative-contract self-tests included); matrix at [`sync-coverage-matrix.md`](sync-coverage-matrix.md), evidence at [`sync-coverage-evidence.json`](sync-coverage-evidence.json) |
