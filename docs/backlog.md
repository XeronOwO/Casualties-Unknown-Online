...
- **Cross-player item use (give/feed/drink/use an item on another player)** — **CLOSED (first slice + medicine + topical, 2026-08-24)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) covers drink/food, curated immediate-effect injectables, and topical `ApplyToLimb` containers; no second wire operation. Wear, timed/component medicine, opiates, and tools remain future slices. See `docs/selfchecks/cross-player-item-use-selfcheck.md`, `docs/selfchecks/cross-player-medicine-use-selfcheck.md`, `docs/selfchecks/cross-player-topical-use-selfcheck.md`, and `docs/tech-decisions.md` #96/#98/#100.
...
- **Other lower-priority KrokMP candidates** — voice, vote-kick, co-op keybinds, status icons, and any remaining player-list polish. Ban is closed as the second admin slice; piggyback and push are closed above. See §2.7.
