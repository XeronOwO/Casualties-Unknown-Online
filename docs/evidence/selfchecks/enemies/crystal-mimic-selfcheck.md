...
| Host apply / guest replay | `CrystalStateActions.ApplyCrystalMimic` writes `activated`; live replay plays the original `observerlaugh` call; late-joiner replay is state-only (silent). Since the shared action verdict (2026-09-13) it answers `TrapActionOutcome`: `AlreadyInState` when the latch was already consumed (a duplicate — the state IS in the world) and `NotApplicable` when this crystal carries no mimic effect at all or the latch member is missing, which is a REFUSED row for the restore account (`TrapActionVerdict`), not a restored one | `CrystalMimic.cs:29/43`, `CrystalMimic.cs:52`; `CrystalBehaviour.SetUpEffects` (the effect list is rolled per crystal, mimic weight 8 of 139, `CrystalBehaviour.cs:83-102`/:196); `TrapEffectApplier`; `TrapVisualReplay`; `CrystalMimicAccess.TryActivate` |
...
- Deploy: `tools/deploy.ps1 -GameDir "<game-dir>"`
