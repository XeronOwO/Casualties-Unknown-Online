...
| Current phase status | In progress — all six Phase D domain areas have kernel foundations; Fluids host production wiring landed; enemy stream lifecycle slice and removal resurrection guard landed; Players carry kernel-wire cutover (kernel command + committed-batch mirror projection, legacy carry-state wire removed), limb terminal facts, and body-level terminal latches landed; WorldEntities guest checkpoint projection landed and the legacy world-entity snapshot wire removed |
