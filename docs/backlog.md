...
- **Large logical class debt flattening** — HIGH (follow-up to the closed gate, in progress). The gate now sees the remaining recorded aggregate debt: `ModService` 1590, `GameAdapter` 1397, `ItemService` 928, `WorldService` 899, `EnemySyncCoordinator` 750. `PlayerInteractionService` (716) and `ItemApplication` (630) were flattened this cycle (2026-08-23): the former split into real top-level `PlayerInventoryTakeService` / `PlayerCarryService` / `PlayerHealService` + shared `PlayerCharacterAccess`; the latter's cook-replay half extracted into `ItemCookReplayApplier`. Both were removed from the debt ledger. Remaining classes must still be split into real top-level responsibilities, not more partial files. See `docs/architecture-debt.json`, `docs/selfchecks/player-interaction-service-split-selfcheck.md`, `docs/selfchecks/item-cook-replay-split-selfcheck.md`, `docs/tech-decisions.md` #66/#67 and §3.1.
...
`SessionService.cs` (580), `ItemApplication.cs` (576), `CharacterDataSync.cs` (563), `EnemySyncCoordinator.cs` (551), `EntitySyncService.cs` (548), `EnemyCombatDirector.cs` (547), `Plugin.cs` (534), `RunCoordinator.cs` (512).
