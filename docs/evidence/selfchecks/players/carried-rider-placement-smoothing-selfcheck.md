...
| `Plugin.LateUpdate` → `ICarryPresentationPump.PinCarriedPresentation` | Runs after every `Update`/`Body.Update` has completed and re-pins every remote rider clone to the carrier's final transform before render |
...
| Carrier-side final before-render pin | every remote rider clone is re-pinned once more after all Update methods, so the rendered frame matches the carrier's final transform | `Plugin.LateUpdate` → `ICarryPresentationPump.PinCarriedPresentation` → `RefreshLocalCarrierAttach` |
