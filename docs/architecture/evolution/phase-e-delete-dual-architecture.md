...
- [x] Update documentation:
  ...
  - `docs/history/architecture-blueprint.md` to reflect the new architecture (or mark the old one superseded and link to the evolution area);
  - `docs/decisions/active.md` with the final removal decisions;
  - `docs/backlog/README.md` to remove completed architecture items;
  - `docs/architecture/evolution/status.md` to mark Phase E complete;
  - `docs/architecture/README.md` to reflect completion.
...
| Docs may still describe old architecture | Update `docs/history/architecture-blueprint.md` and root README in this phase, not later. |
...
|---|---|---|---|---|
| 2026-08-30 | Phase E start: legacy inventory + remove dead `ItemCheckpointStore` | current | 1792 tests green; build/format/architecture/event/entity/delivery gates pass | Inventory recorded in `docs/evidence/selfchecks/architecture/phase-e-legacy-inventory-selfcheck.md`; removed the Phase B temporary in-memory checkpoint store and its DI registration/tests. |
...
| 2026-08-30 | Phase E third batch: add no-legacy architecture guard | current | Architecture gate passed with the new `check-no-legacy.ps1` scan | Created `tools/check-no-legacy.ps1`, wired it into `tools/check-architecture.ps1`, and documented the Phase E addendum in `docs/architecture/guards.md`. |
