...
- **Cross-player item use (give/feed/drink/use an item on another player)** — **CLOSED (first slice + medicine + topical + opiates + limb-tool first slice, 2026-08-25)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) covers drink/food, curated immediate-effect injectables, topical `ApplyToLimb` containers, curated injectable opiates/opiate-antagonists (`morphine`/`opium`/`heroin`/`fentanyl`/`naloxone`), and the curated non-liquid limb-tool set (`boneweldingtool`/`clottingmush`/`chestdrain`/`musharm`); no second wire operation. The opiate `Painkillers` component state now rides `CharacterHealthMsg` (additive fields, no protocol bump). Wear, timed/component medicine, and component-bearing tools (splint/tourniquet/icepack/etc.) remain future slices. See `docs/selfchecks/cross-player-item-use-selfcheck.md`, `docs/selfchecks/cross-player-medicine-use-selfcheck.md`, `docs/selfchecks/cross-player-topical-use-selfcheck.md`, `docs/selfchecks/cross-player-opiate-use-selfcheck.md`, `docs/selfchecks/cross-player-limb-tool-use-selfcheck.md`, and `docs/tech-decisions.md` #96/#98/#100/#102/#103.
