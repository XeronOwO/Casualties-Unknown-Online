...
| Current phase status | In progress — World/Run/Epoch, WorldEntities, Players, and Enemy/Entity kernel baselines landed |
| Last status update | 2026-08-29 |
| Next work session | Continue Phase D: add enemy combat/bite/lunge terminal batches, then move to Fluids persistent region domain. |
| Protocol/save compatibility | The new four-envelope protocol and checkpoint save stack are the production item paths. Old item packet handlers/DTOs and the corresponding `NetMsg` item enums have been fully removed; the only legacy item-frame survivor is `ItemReject` for block-break drop refusal. World/Run, WorldEntities, Players, and Enemy facts ride the kernel/checkpoint; player/entity host writes project into the kernel. |
...
| D — Full domain migration | In progress (World/Run/Epoch, WorldEntities, Players, Enemy/Entity baselines) | 2026-08-29 | World/Run/Epoch and WorldEntities authority switches complete. Players terminal-status and Enemy/Entity lifecycle-health kernel plus production projections landed. 1662 tests green. See the phase-d selfcheck set in `docs/selfchecks/`. |
