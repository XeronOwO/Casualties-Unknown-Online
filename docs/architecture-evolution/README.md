...
> Status: **Architecture evolution complete**. Phases A–E are complete: the
> typed deterministic kernel is authoritative across all migrated domains,
> the protocol/save stack is kernel-driven, dual-architecture surfaces are
> gone, and the Phase E guard suite is in place. See
> [status.md](status.md), `docs/selfchecks/phase-e-legacy-inventory-selfcheck.md`,
