...
/// alive/conscious, discrete limb latches, body-level terminal latches, and the
/// carry relation; the legacy character snapshot stays the authority for
/// continuous physiological fields, skills, items, and position. Carry is not a
/// character-snapshot field and is restored separately through
	...
	ILogger<PlayerKernelRestoreProjection> log)
{
	...
	public void Apply(ulong steamId, CharacterDataMsg data)
	{
		...
		if (player is null)
		{
			_log.LogDebug("[PlayerKernelRestore] no kernel terminal facts for {Player}; snapshot remains the fallback.", steamId);
			return;
		}
		...
		_log.LogDebug("[PlayerKernelRestore] projected kernel terminal facts for {Player}.", steamId);
	}
