...
| 4 | A stale `BlockDamageReport` row set for a regenerated cell | Not applied to the new layer's block | `GuestBlockDamageReportRecoveryTests.ReportFromAnotherGeneration_IsNeitherAppliedNorAnswered` (0 merge calls, no answer, the reporter's entry stays its own boundary's business) |
