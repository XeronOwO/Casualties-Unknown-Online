...
|---|---|---|
| 1 | Id value type | `ContentId` (Abstractions) — `TryParse`/`TryCreate`/`IsValidNamespace`/`IsValidPath`/`Parse`, value equality, ordinal ordering, canonical `ToString`. Grammar `[a-z][a-z0-9_]{0,31}` / `[a-z0-9][a-z0-9_.-]{0,94}` derived from the vanilla item-id charset (`reversing/Assembly-CSharp/Assembly-CSharp/Item.cs`, 218 ids, `a-z0-9`) and the existing 128-char content-id cap. |
