...
- **Cross-player item use (give/feed/drink/use an item on another player)** — **CLOSED (first slice + medicine slice, 2026-08-24)**. Host-authoritative `PlayerItemUseRequest`/`PlayerItemUseResult` (NetMsg 116/117, ProtocolVersion 48) covers drink/food plus curated immediate-effect medicine containers (saline/ringer/bloodbags/antiserum/ceftriaxone/streptokinase); no second wire operation. Wear, timed/component medicine, and tools remain future slices. See `docs/selfchecks/cross-player-item-use-selfcheck.md`, `docs/selfchecks/cross-player-medicine-use-selfcheck.md`, and `docs/tech-decisions.md` #96/#98.
...
- **Other lower-priority KrokMP candidates** — voice, vote-kick, co-op keybinds, push, status icons, and any remaining player-list polish. Ban is closed as the second admin slice; piggyback is closed above (push remains future). See §2.7.
