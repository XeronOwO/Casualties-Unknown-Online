...
| Fluids | `FluidStateTable` / `FluidRegionState` | `UpdateFluidRegionCommand`, `ResetFluidsCommand` (no production caller today — the stale chunks are cleared by zero-total upserts from `FluidKernelProjection`) | `FluidRegionUpdatedEvent`, `FluidsResetEvent` | Coarse authoritative region totals/checkpoints, not every fluid pixel | `FluidKernelProjection`, `FluidKernelReadProjection` |
